...
- **Flattened JAR:** `target/injector-<version>.jar` (e.g. `target/injector-9.5-SNAPSHOT.jar`) — runnable with `java -jar injector-<version>.jar [options]`.
...
|-----------------------------|--------------------------------------------------------|
| `dadoonet/persons-injector` | `<project.version>` (e.g. `9.5-SNAPSHOT`) and `latest` |
...
docker run dadoonet/persons-injector:9.5-SNAPSHOT
...
```bash
mvn clean package -DskipTests
java -jar target/injector-9.5-SNAPSHOT.jar --console --nb 10
```
...
docker run --rm dadoonet/persons-injector:9.5-SNAPSHOT --console --nb 5
```
...
```bash
docker run --rm dadoonet/persons-injector:9.5-SNAPSHOT --console --nb 5 --cs.pretty
```
...
| Run JAR in console mode      | `java -jar target/injector-9.5-SNAPSHOT.jar --console --nb 10`            |
| Check Docker image (minimal) | `docker run --rm dadoonet/persons-injector:9.5-SNAPSHOT --console --nb 5` |

All of the above should be run from the project root directory. Replace `9.5-SNAPSHOT` with your actual project version if different (see `pom.xml` or `mvn help:evaluate -Dexpression=project.version -q -DforceStdout`).
