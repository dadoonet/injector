...
- **Flattened JAR:** `target/injector-<version>.jar` (e.g. `target/injector-9.3.1.jar`) — runnable with `java -jar injector-<version>.jar [options]`.
...
|-----------------------------|--------------------------------------------------------|
| `dadoonet/persons-injector` | `<project.version>` (e.g. `9.3.1`) and `latest` |
...
docker run dadoonet/persons-injector:9.3.1
...
```bash
mvn clean package -DskipTests
java -jar target/injector-9.3.1.jar --console --nb 10
```
...
docker run --rm dadoonet/persons-injector:9.3.1 --console --nb 5
```
...
```bash
docker run --rm dadoonet/persons-injector:9.3.1 --console --nb 5 --cs.pretty
```
...
| Run JAR in console mode      | `java -jar target/injector-9.3.1.jar --console --nb 10`            |
| Check Docker image (minimal) | `docker run --rm dadoonet/persons-injector:9.3.1 --console --nb 5` |

All of the above should be run from the project root directory. Replace `9.3.1` with your actual project version if different (see `pom.xml` or `mvn help:evaluate -Dexpression=project.version -q -DforceStdout`).
