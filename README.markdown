...
https://s01.oss.sonatype.org/content/repositories/snapshots/fr/pilato/elasticsearch/injector/injector/8.4/

```sh
# Download it from maven central
wget https://repo1.maven.org/maven2/fr/pilato/elasticsearch/injector/8.4/injector-8.4.jar

# Launch it (with all default settings)
java -jar injector-8.4.jar
```
...
```sh
java -jar injector-8.4.jar --nb 1000 --bulk 100
```
...
```sh
java -jar injector-8.4.jar --elasticsearch --es.host https://cloud_id.europe-west1.gcp.cloud.es.io:9243
```
...
```sh
java -jar injector-8.4.jar --elasticsearch --es.user elastic --es.pass changeme
```
...
```sh
java -jar injector-8.4.jar --elasticsearch --es.index person
```
...
```sh
java -jar injector-8.4.jar --appsearch --ap.host http://localhost:3002/api/as/v1/ --es.user elastic --es.pass changeme
```
...
```sh
java -jar injector-8.4.jar --appsearch --ap.host http://localhost:3002/api/as/v1/ --es.user elastic --es.pass changeme --ap.engine person
```
...
```sh
java -jar injector-8.4.jar --console --cs.pretty
```
...
```sh
java -jar injector-8.4.jar --console --cs.appsearch
```
...
Just get the final jar from `target/injector-8.4.jar`. Or:
