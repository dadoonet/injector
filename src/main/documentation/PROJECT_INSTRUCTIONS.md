...
docker run dadoonet/persons-injector:${project.version}
...
```bash
mvn clean package -DskipTests
java -jar target/injector-${project.version}.jar --console --nb 10
```
...
docker run --rm dadoonet/persons-injector:${project.version} --console --nb 5
```
...
```bash
docker run --rm dadoonet/persons-injector:${project.version} --console --nb 5 --cs.pretty
```
...
| Run JAR in console mode      | `java -jar target/injector-${project.version}.jar --console --nb 10`            |
| Check Docker image (minimal) | `docker run --rm dadoonet/persons-injector:${project.version} --console --nb 5` |

All of the above should be run from the project root directory. Replace `${project.version}` with your actual project version if different (see `pom.xml` or `mvn help:evaluate -Dexpression=project.version -q -DforceStdout`).
